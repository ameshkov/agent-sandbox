...
- **Never try to install or start a local engine** (`dockerd`, Docker
  ...
  host and/or re-run `./scripts/run-ubuntu-vmware-sandbox.sh`, and
  continue with work that does not need Docker.
...
- After editing opencode or Copilot settings in the VM, restart OpenChamber
  with `systemctl --user restart agent-sandbox-openchamber` (the runner
  restarts it automatically when it brings the bridges up). To pull the
  host's versions of the settings instead, the user re-runs
