#!/bin/bash
#
# delete-windows-vmware-sandbox.sh — delete the Windows 11 (ARM64) VMware
...
#      stop-windows-vmware-sandbox.sh ('vmrun stop' + the socat bridge
...
#      run-windows-vmware-sandbox.sh re-pulls the archive and re-clones.
...
usage() {
    cat <<'EOF'
Usage: delete-windows-vmware-sandbox.sh [options]

Deletes the Windows 11 (ARM64) VMware sandbox state: stops the working VM
(delegating to stop-windows-vmware-sandbox.sh), then removes the state dir
(extracted pristine base + working clone + pulled image cache). The next
run-windows-vmware-sandbox.sh re-pulls the archive and re-clones.
...
title "Deleting Windows VMware sandbox"
...
"$repo_root/scripts/stop-windows-vmware-sandbox.sh" || true
...
info "Next run: ./scripts/run-windows-vmware-sandbox.sh (re-pulls the archive)"
