#!/bin/bash
#
# stop-ubuntu-vmware-sandbox.sh — stop the Ubuntu 24.04 (ARM64) VMware
...
#   2. Kills the host socat bridges that run-ubuntu-vmware-sandbox.sh
...
#      guest-side bridges (systemd user services) stop with the VM.
#
...
work_vmx="$host_state_dir/working/sandbox-ubuntu-24-04-arm64-vmware.vmx"

repo_root=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)

# --- shared library ---------------------------------------------------------
#
# scripts/lib/vmware.sh: vmrun resolution (PATH > Fusion app bundle) and
# the vmrun() wrapper.
...
usage() {
    cat <<'EOF'
Usage: stop-ubuntu-vmware-sandbox.sh [options]

Stops the Ubuntu 24.04 (ARM64) VMware sandbox VM (vmrun stop, graceful
with a hard power-off fallback) and kills the host SSH agent / Docker
bridge listeners that run-ubuntu-vmware-sandbox.sh left up.
...
title "Stopping Ubuntu VMware sandbox"
