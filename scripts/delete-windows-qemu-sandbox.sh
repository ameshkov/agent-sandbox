#!/bin/bash
#
# delete-windows-qemu-sandbox.sh — delete the Windows 11 (ARM64) QEMU sandbox
# state from the host.
#
# Usage:
...
#
# What it does:
#   1. Stops qemu, swtpm and the host bridges by delegating to
#      stop-windows-qemu-sandbox.sh (via the runner's pid files) — a delete
#      must never race a running VM.
#   2. Removes $SANDBOX_STATE_DIR recursively: the COW overlay (the working
#      disk), the persistent TPM state and EFI NVRAM, and the pulled image
#      cache. The next run-windows-qemu-sandbox.sh re-pulls the image and
#      starts fresh.
...
usage() {
    cat <<'EOF'
Usage: delete-windows-qemu-sandbox.sh [options]

Deletes the Windows 11 (ARM64) QEMU sandbox state: stops qemu + swtpm
(delegating to stop-windows-qemu-sandbox.sh), then removes the state dir
(working disk overlay, TPM, EFI NVRAM and the pulled image cache). The next
run-windows-qemu-sandbox.sh re-pulls the image and starts fresh.
...
title "Deleting Windows QEMU sandbox"

# 1. Stop the sandbox (qemu + swtpm + host bridges). The stop script is
#    idempotent, so it is safe when the sandbox is already stopped; its
#    non-zero exits are warnings here — the deletion is what matters.
step "Stopping the sandbox"
"$repo_root/scripts/stop-windows-qemu-sandbox.sh" || true

# 2. Remove the state dir (working disk + TPM + NVRAM + image cache).
...
if [ ! -e "$host_state_dir" ]; then
    ...
    if confirm_delete \
        "Delete the sandbox state at '$host_state_dir' (${size:-~20 GB}; re-pulled on the next run)?" y; then
...
info "Next run: ./scripts/run-windows-qemu-sandbox.sh (re-pulls the image)"
